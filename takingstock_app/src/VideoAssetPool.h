...
struct VideoEntry {
    ...
    int videoWidth  = 0;           // pixel width from CSV width column (0 if not present)
    int videoHeight = 0;           // pixel height from CSV height column (0 if not present)
    std::string baseVideoId;       // filename with _scale_<digits> stripped — groups scale variants together
};
...
public:
    ...
    // picks from unused; returns full entry incl. clusterNo.
    // slotW/slotH: actual slot pixel dimensions for scale-aware selection (0 = disabled).
    VideoEntry getVideoEntry(int wr, int hr, int slotW = 0, int slotH = 0);
    // like getVideoEntry but only picks videos with duration >= minDuration; returns empty if none available (pool unchanged)
...
private:
    ...
    // When scaleSelectEnabled: pick optimal scale variant for (slotW x slotH) from a group of indices.
    // Returns the best index into videos[], or fallback to the original vidIdx if no width/height data.
    size_t pickBestScaleVariant(size_t vidIdx, int slotH) const;
    // Removes all entries whose index is in variantIndices from the available vector.
    ...
    std::map<std::string, std::vector<size_t>> scaleVariantsByBase;  // baseVideoId -> all indices with that base

...
{
    if (n < 1 || n > 6 || (int)ratiosOnly.size() < n) return {};
    if (n == 1) return {{0, 0, boxW, boxH}};
        ...
        for (int attempt = 0; attempt < 50; ++attempt) {
            ...
            if (valid && (int)placements.size() == n) {
                for (size_t i = 0; i < placements.size() && valid; ++i) {
                    ...
                    if (std::abs(aspectCurr - aspectTarget) / aspectTarget > 0.12f)
                        valid = false;
                }
                if (valid) {
...
int BinSorter::getMaxGapInNestedData(const NestedBinData& nd) const {
    return getMaxGapInNestedData(nd, nd.parentW, nd.parentH);
}

int BinSorter::getMaxGapInNestedData(const NestedBinData& nd, int parentW, int parentH) const {
    int maxArea = getLargestEmptyRectArea(nd.items, parentW, parentH);
    ...
    for (size_t bi = 0; bi < bins.size(); ++bi) {
        ...
        for (const auto& it : bins[bi]) {
            ...
            if (nit != nestedBins.end()) {
                // Use parent's actual dimensions from bins (may have been expanded by gapFillPass/stretchToEdges)
                int deep = getMaxGapInNestedData(nit->second, it.w, it.h);
                if (deep > maxArea) maxArea = deep;
            }
        }
    }
...
std::string BinSorter::getLayoutSignature() const {
    ...
    for (size_t bi = 0; bi < bins.size(); ++bi) {
        ...
        for (size_t ii = 0; ii < bins[bi].size(); ++ii) {
            ...
            sig += std::to_string(it.w) + "x" + std::to_string(it.h) + "@" + std::to_string(it.x) + "," + std::to_string(it.y);
            auto nit = nestedBins.find({(int)bi, it.itemIdx});
            if (nit != nestedBins.end()) {
                sig += "[";
                for (size_t ni = 0; ni < nit->second.items.size(); ++ni) {
                    ...
                    sig += std::to_string(n.w) + "x" + std::to_string(n.h) + "@" + std::to_string(n.x) + "," + std::to_string(n.y);
                }
                sig += "]";
            }
        }
...
std::vector<std::vector<BinItem>> BinSorter::sortInfinite() {
    ...
    while (true) {
        ...
        if (doBreak) {
            ...
            ratiosOnly.push_back({wr, hr});  // include parent ratio so e.g. 4x 2x3 items fit exactly in 2x3 box

            int minN = std::max(1, breakBoxMinItems);

	...
	dispatch_async(queue, ^{
		...
		[asset loadValuesAsynchronouslyForKeys:[NSArray arrayWithObject:kTracksKey] completionHandler:^{
			...
			if (!self->bStream) {
				...
				NSString *filename = [(AVURLAsset *)self.asset URL].lastPathComponent ?: @"(unknown)";
				NSLog(@"video file loaded: %@ at %li x %li @ %.2f fps", filename, (long)self->videoWidth, (long)self->videoHeight, (double)self->frameRate);
			}
